...
public class IngredientAnalyzer {
	...
	public IngredientAnalyzeResult analyze(IngredientSource ingredientSource) {
		IngredientType mostIncludedIngredient = ingredientSource.getIngredients().stream()
			.sorted()
			.findFirst()
			.orElseThrow();

		String ingredientName = mostIncludedIngredient.getName();

		String title = String.format("%s는 물세탁이 가장 좋은 섬유입니다.", ingredientName);
		String description = String.format(
			"%s 특성상 마찰에 의한 보풀이 잘 일어날 수 있는 섬유이기 때문에 손세탁으로 세탁하시면 좋습니다. 세제는 가능하면 중성세제나 약산성 전용세제를 사용하시고, 세탁은 무조건 실온물을 사용하시면 안전합니다.",
			ingredientName);
